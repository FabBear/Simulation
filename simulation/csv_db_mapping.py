...
# CSV filename → KPI level (column not present in split CSV files)
...
CSV_TO_TABLE: dict[str, str] = {
    ...
    **{k: "kpi_snapshot" for k in KPI_FILE_TO_LEVEL},
}

# Optional files (missing file → skip with warning)
OPTIONAL_CSV_FILES = frozenset({"simulation_process.csv"})
...
def map_csv_row(filename: str, row: dict[str, str], run_id: str) -> dict[str, Any]:
    ...
    if filename in KPI_FILE_TO_LEVEL:
        out["level"] = KPI_FILE_TO_LEVEL[filename]
    return out
